...
export async function programInitMenu() {
    ...
    if (!fs.existsSync(hcFilePath)) {
        const hcUploadedFilePath = await filer.createFolderAndWaitUpload({
            filePattern: /.+\.xlsx$/,
            message: ck.yellow("Adicione a planilha de HC na pasta aberta"),
        });
        if (!hcUploadedFilePath)
            return;
    ...
    switch (rateioType) {
        case "voll":
            const vollSheetUploaded = await filer.createFolderAndWaitUpload({
                filePattern: /.+\.xlsx$/,
                message: ck.yellow("Adicione a planilha de rateio Voll na pasta aberta"),
            });
            if (!vollSheetUploaded)
                return;
            const spinner = createSpinner();
            spinner.start("🔄 Processando planilha...");
            try {
                const vollWorkbook = XLSX.readFile(vollSheetUploaded);
                const vollData = XLSX.utils.sheet_to_json(vollWorkbook.Sheets[vollWorkbook.SheetNames[0]], { header: 1 });
                const hcWorkbook = XLSX.readFile(hcFilePath);
                const hcData = XLSX.utils.sheet_to_json(hcWorkbook.Sheets[hcWorkbook.SheetNames[0]], { header: 1 });
                    ...
                    if (cc && service && tax && passengerName) {
                        ...
                        if (!isNaN(taxValue) && !isNaN(totalValue)) {
                            ...
                            if (!found) {
                                warnings.push({
                                    type: "notFound",
                                    message: ck.red(`❌ Passageiro ${passengerName} não encontrado na planilha de HC`)
                                });
                            }
                        }
                    }
                ...
                for (const entry of ccData) {
                    const { cc, commission, type } = entry;
                    let svCode = "";
                    let typeDescription = "";
                    if (type === "Despesa") {
                        svCode = commission > 0 ? `SV.190` : "";
                        typeDescription = "Compra Despesa";
                    }
                    else if (type === "Custo") {
                        svCode = commission > 0 ? `SV.194` : "";
                        typeDescription = "Compra Custo";
                    }
                    if (entry.service === "AÉREO/RODOVIÁRIO/CARRO") {
                        svCode = commission > 0 ? `SV.189` : "";
                    }
                    else if (entry.service === "HOTEL") {
                        svCode = commission > 0 ? `SV.195` : "";
                    }
                    sapData.push([svCode, "", "1", commission, "0", typeDescription, "Não", "1933-005", "1,933", "", "", "", "", "Manual", "", "0", cc, "", "SYMPLA INTERNET SOLUCOES S/A", "96", "", "", "", "", "0", "0", "", "", "", "", "0"]);
                                    ...
                                    if (similarity > 0.6) {
                                        for (const [rowIndex, row] of vollData.entries()) {
                                            if (row[15] === passengerName) {
                                                row[11] = hcCC;
                                                vollData[rowIndex][26] = parseFloat((parseFloat(row[24]) - parseFloat(row[18])).toFixed(2));
                                            }
                                        }
                                    }
                                }
